...
class vfsStreamTestCase extends \PHPUnit_Framework_TestCase {
    ...
    public function setupArraysAreTurnedIntoSubdirectories() {
        ...
        $this->assertInstanceOf('org\\bovigo\\vfs\\vfsStreamDirectory', $root->getChild('test')
        );
    ...
    public function setupWorksRecursively() {
        $root = vfsStream::setup('root', null, array('test' => array('foo' => array('test.txt' => 'hello'),
                        'baz.txt' => 'world'
                    )
                        )
        );
    ...
    public function createArraysAreTurnedIntoSubdirectories() {
        ...
        $this->assertInstanceOf('org\\bovigo\\vfs\\vfsStreamDirectory', $baseDir->getChild('test')
        );
    ...
    public function createWorksRecursivelyWithRoot() {
        $root = vfsStream::setup();
        $this->assertSame($root, vfsStream::create(array('test' => array('foo' => array('test.txt' => 'hello'),
                        'baz.txt' => 'world'
                    )
                        )
                )
    ...
    public function copyFromPreservesFilePermissions() {
        ...
        $this->assertEquals(fileperms($copyDir . '/withSubfolders') - vfsStreamContent::TYPE_DIR, $root->getChild('withSubfolders')
                        ->getPermissions()
        );
        $this->assertEquals(fileperms($copyDir . '/withSubfolders/aFile.txt') - vfsStreamContent::TYPE_FILE, $root->getChild('withSubfolders/aFile.txt')
                        ->getPermissions()
        );
    }
