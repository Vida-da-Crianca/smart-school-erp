...
class Staff extends Admin_Controller {
    ...
    /*public function curriculo_deletar($id){
        ...
        if($result){
            $this->session->set_flashdata('msg', '<div class="alert alert-danger">O C�rriculo n�o foi exclu�do. Tente novamente mais tarde.</div>');
        } else {
    ...
    public function salvar_curriculo($action){
        ...
        if($this->input->is_ajax_request() && !empty($this->input->post('id')) && ($action == 'salvar' || $action == 'efetivar')){
            try{
                ...
                $this->form_validation->set_rules('emergency_no', utf8_encode('Telefone de Emerg�ncia'), 'trim|required|xss_clean');
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    if($action == 'salvar'){
                        ...
                        $data['foto'] = $this->resp_foto;

                        $result_update = $this->curriculo_model->update($id, $data);

                        if($result_update){
                            $this->resp_status = true;
                            $this->resp_msg = utf8_encode("Altera��es salvas com sucesso!");
                        } else {
                            $this->resp_status = false;
                            $this->resp_msg = utf8_encode("N�o foi poss�vel salvar os dados. Tente novamente.");
                        }
                        ...
                        if($this->resp_status)
                            ...
                            $this->resp_msg = utf8_encode("N�o foi poss�vel efetivar o candidato no momento. Tente novamente.");
...
//        $countAttendance = $this->countAttendance($start_year, $startMonth, $id);
    ...
    public function create() {
        ...
        $this->form_validation->set_rules('numero', utf8_encode('N�mero da casa'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('role', $this->lang->line('role'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('gender', $this->lang->line('gender'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('dob', $this->lang->line('date_of_birth'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('file', $this->lang->line('image'), 'callback_handle_upload');
