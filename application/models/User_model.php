...
class User_model extends MY_Model {
    ...
    public function getStudentLoginDetails($student_id) {
        ...
        $sql = "SELECT users.*, CONCAT_WS(', ', IF(users.childs <> '0', students.guardian_document, users.username))  as username
            FROM users INNER JOIN students on students.id =users.user_id or students.id = users.childs  WHERE users.id in (select students.parent_id from users INNER JOIN students on students.id =users.user_id WHERE users.user_id=" . $this->db->escape($student_id) . "
             AND users.role ='student') UNION select users.*, CONCAT_WS(', ', IF(users.childs <> '0', students.guardian_document, users.username))  as username from users INNER JOIN students on students.id =users.user_id WHERE users.user_id=" . $this->db->escape($student_id) . " 
